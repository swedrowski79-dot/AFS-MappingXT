...
SecurityValidator::validateAccess($config, 'debug.php');
...
// Build local debugTables like index.php
$debugTables = [
...
<head>
  ...
  <link rel="stylesheet" href="<?= htmlspecialchars($baseUrl, ENT_QUOTES, 'UTF-8') ?>assets/css/main.css">
...
<body>
  <div class="shell">
    <header>
      ...
      <div class="tag">
        <a href="<?= htmlspecialchars($baseUrl, ENT_QUOTES, 'UTF-8') ?>" class="back-link">← Zurück</a>
      </div>
    </header>
      ...
      <div class="server-selector-section">
        <div class="server-selector-header">
          ...
          <div class="server-selector-controls">
            <select id="dbg-server" class="server-select">
  ...
  <script src="<?= htmlspecialchars($baseUrl, ENT_QUOTES, 'UTF-8') ?>assets/js/debug.js"></script>
</body>
</html>
